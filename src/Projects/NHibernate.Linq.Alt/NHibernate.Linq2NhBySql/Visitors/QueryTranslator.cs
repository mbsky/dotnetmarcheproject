...
namespace NHibernate.Linq2NhBySql.Visitors
{
	public class QueryTranslator : ExpressionVisitor {
		...
		public void Translate(Expression exp) {
			...
			Linq.Visitors.Select.GroupVisitor gvisitor = new Linq.Visitors.Select.GroupVisitor(row, CurrentColumnIndex);
	...
	public class QueryTranslated {
		...
		private object GetProjectedResult<TResult>() {
			if (Criteria.Projection.IsGrouped) {
				...
				return Activator.CreateInstance(
					typeof(Linq.Visitors.Select.ProjectionReader<>).MakeGenericType(typeToReturn),
